...
public class AircraftPhysics : MonoBehaviour
{
	...
	private float fff2 = 0f;
	private float fff3 = 0f;
    private Vector3 PredictAngularVelocity(Vector3 torque)
    {
		if (Input.GetKey(KeyCode.UpArrow)){
			Vector3 rotate = new Vector3(0, fff2, 0);
			if (fff2 >= 3f){
				fff3 += 1f;
				rb.AddForce (0, fff3, 0, ForceMode.Impulse);
			}
			else{
				fff2 += 0.01f;
			}

			Quaternion deltarotation = Quaternion.Euler(rotate);
